...
if __name__ == '__main__':
    data_m4 = load_data('m5.txt')
    data_x = [i for i in range(0, 280, 20)]

    plt.style.use('seaborn-darkgrid')
    plt.figure(figsize=(9.6, 7.2))
    plt.plot(data_x, data_m4, marker='.', color='black', linewidth=1.5, alpha=0.7)
    plt.title("memcached Server 95th Percentile Latency of Run 2", loc='center', fontsize=16, fontweight=1, color='black')
    ...
    plt.text(2, 0.46, 'dedup\nferret\nfreqmine\ncanneal\nfft', color='orange', fontsize='x-large')
    plt.axvline(x=53, linewidth=2, color='orange')
    plt.text(55, 0.46, 'blackholes', color='orange', fontsize='x-large')
